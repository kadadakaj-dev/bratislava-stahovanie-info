 ...
 */
// Title / description builder helpers
const buildBaseTitle = (core: string, brand = 'VI&MO') => `${core} | ${brand}`;
const buildServiceTitle = (service: string) => buildBaseTitle(service);
const buildDistrictTitle = (district: string) => buildBaseTitle(`Sťahovanie ${district}`);
const buildDescription = (parts: string[]) => parts.filter(Boolean).join(' ').slice(0, 300);

// Keyword meta composer (keeps length reasonable, deduped)
const buildKeywordMeta = () => {
    const raw = [KEYWORDS.primary, ...KEYWORDS.secondary, ...KEYWORDS.longtail.slice(0,5)];
    return Array.from(new Set(raw)).join(', ');
};

const updateSeoTags = (view: View, t: Translations, post?: Post) => {
    let title = buildBaseTitle(t.seoTitleHome);
    let description = t.seoDescHome;
    let canonicalUrl = BASE_URL;
    let imageUrl = "https://picsum.photos/seed/stahovanie1/1200/630";
    let keywords = buildKeywordMeta();

    switch(view) {
        case 'pricing':
            title = buildBaseTitle(t.seoTitlePricing);
            description = t.seoDescPricing;
            canonicalUrl = `${BASE_URL}/#pricing`;
            break;
        case 'references':
            title = buildBaseTitle(t.seoTitleReferences);
            description = t.seoDescReferences;
            canonicalUrl = `${BASE_URL}/#references`;
            break;
        case 'about':
            title = buildBaseTitle(t.seoTitleAbout);
        ...
        case 'blog':
            if (post) {
                title = buildBaseTitle(`${post.title}`);
                description = post.excerpt;
                canonicalUrl = `${BASE_URL}/#blog/${post.id}`;
                imageUrl = post.imageUrl;
                keywords = post.tags.join(', ');
            } else {
                title = buildBaseTitle(t.seoTitleBlog);
